    <footer>
        <div class="container-fluid bg-dark text-white">
            <div class="row">
                <!-- contenedor enlaces -->
                <div class=" text-center col-sm-12 col-md-6 col-lg-4">
                    <a href="/20192B106/index.php" title="Ir al inicio">Inicio</a> /
                    <a href="acerca_de.php" title="Que es la litografía Angarita Ardila">Acerca de nosotros</a> /
                    <a href="ubicacion.php" title="Donde esta la litografía Angarita Ardila">Ubicación</a> /
                    <a href="contactenos.php" title="Contacta con nosotros">Contactenos</a> /
                    <a href="preguntas.php" title="Preguntas Frecuentes">Preguntas Frecuentes</a> /
                    <a href="matriz.php" title="Formulario">Multiplicar matrices</a>/
                    <a href="subir.php" title="Formulario">Subir archivos</a>/
                    <a href="tablas.php" title="Formulario">Ir a crud</a>
                </div>
                <!-- contenedor informacion general -->
                <div class=" text-center col-sm-12 col-md-6 col-lg-4">
                    <strong>PBX:</strong> 6 42 98 10 <strong>|</strong> Diagonal
                    15 #45 - 178 CC. La rosita primer piso <strong>|</strong> Bucaramanga - Colombia <strong>|</strong>
                    www.litoangarita.com <strong>|</strong>
                    angaritaardilaimpresores@hotmail.com
                </div>
                <!-- contenedor botones redes sociales -->
                <div class="d-flex align-self-center justify-content-center col-sm-12 col-md-12 col-lg-4">
                    <div>
                        <a href="https://www.facebook.com/wilson.angaritasalazar" class="btn btn-outline-primary">FACEBOOK</a>
                        <a href="https://www.instagram.com/angaritaardilaimpresores95/" class="btn btn-outline-danger">INSTAGRAM</a>
                    </div>
                </div>
            </div>
        </div>
    </footer>
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script src="https://code.jquery.com/jquery-3.4.1.slim.min.js" integrity="sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
    <script src="../js/bootstrap.bundle.js"></script>
    </body>

    </html>